...
(* your rules go here *)
  ...
  | "fun" { FUN }
	...
	| "," { COMMA }
	| "_" { UNDERSCORE }
	| (numeric+) as n { INT(int_of_string n) }
	| (numeric+)'.'(numeric*) as f { FLOAT(float_of_string f) }
	| "true" { BOOL(true) }
	| "false" { BOOL(false) }
	| "()" { UNIT }
	| lowercase+letter*numeric* as s { IDENT(s) }
	| "//"[^'\n']*['\n'] { token lexbuf }
	...
	| "**)" { token lexbuf }
	| "(*" { lex_comment (n+1) lexbuf }
	| "*)" { if n > 0 then lex_comment(n-1) lexbuf else token lexbuf }
	| _ { lex_comment n lexbuf }
	...
	| "\\" (['0' - '9']['0' - '9']['0' - '9'] as x) { (String.make 1 (char_of_int (int_of_string x))) ^ (lex_string lexbuf) }
	| "\"" { "" }
